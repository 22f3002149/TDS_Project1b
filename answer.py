...
def generate_llm_respose2(question: str, context: str):
    ...
    data = {
        "model": "openai/gpt-3.5-turbo-0125",  # 'text-embedding-3-small' is for embeddings, not chat. Use chat model for text generation.
        "messages": [
            {"role": "system", 
...
def answer(question: str, image: str = None):
    ...
    question_embedding = get_embedding2(question)
    ...
    response  = generate_llm_respose2(question, "\n".join(top_chunks))
    return {
        "question": question,
        "response": response,
